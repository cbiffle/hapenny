...
class TestMemory(Component):
    ...
    inspect_response: Out(StreamSig(32))
    ...
    def elaborate(self, platform):
        ...
        m.d.comb += [
            inspect_rp.addr.eq(self.inspect_command.payload.addr[2:]),
            inspect_rp.en.eq(self.inspect_command.valid &
                             (self.inspect_command.payload.lanes == 0)),

            inspect_wp.addr.eq(self.inspect_command.payload.addr[2:]),
            ...
            inspect_wp.en[2].eq(self.inspect_command.valid &
                                self.inspect_command.payload.lanes[2]),
            inspect_wp.en[3].eq(self.inspect_command.valid &
                                self.inspect_command.payload.lanes[3]),
...
def write_mem(addr, value):
    ...
    yield mem.inspect_command.payload.data.eq(value)
    yield mem.inspect_command.payload.lanes.eq(0b1111)
...
def read_mem(addr):
    ...
    yield mem.inspect_command.valid.eq(0)
    yield Settle()
    return (yield mem.inspect_response.payload)
